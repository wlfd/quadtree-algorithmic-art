...
private:
    /* helper for stats constructor */
    long buildSumTable(vector< vector<long> > *sumTable, long currVal, int i, int j);

    /* helper for getSum and getSumSq */
    long findSum(pair<int, int> ul, int w, int h, vector< vector<long> > *sumTable);
};
